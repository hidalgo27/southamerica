...
<template>
  <section>
      ...
      <div class="overflow-hidden h-full relative ">
        <div class="mask gradient-cicle-gray items-center flex justify-center -z-20">
          <NuxtImg v-if="computedHeader.url" :src="computedHeader.url" alt="Header Image"
            class="absolute inset-0 w-full h-full object-cover" :class="{ 'brightness-90': !route.params.package }" />
          <div v-else>
        ...
        <div class="relative z-10 flex items-center w-full h-full text-center">
          ...
          <div v-else
            class="container hidden lg:block absolute bottom-10 left-4 md:left-10 w-11/12 md:w-2/3 lg:w-1/3 2xl:w-1/4 bg-blue-800/80 text-white p-4 rounded-md shadow-lg"
    ...
    <div v-if="packageDetail" class="container lg:hidden my-6 w-11/12 bg-blue-800 text-white p-4 rounded-md shadow-lg"
      v-for="paquete in packageDetail">
      <div class="flex justify-between items-center border-gray-100 border-b">
        <div class="items-center rounded flex flex-wrap gap-2 py-4 text-start">
          <span v-if="paquete.offers_home"
          ...
          <NuxtLink v-if="paquete.paquetes_destinos" v-for="(region, i) in paquete.paquetes_destinos"
            :to="/destinations/ + region.destinos.url"
      ...
      <div class="text-white">
        ...
        <div v-if="!paquete.travelGuide" class="flex text-start items-center border-t pt-4 text-sm">
          ...
          <div class="block border-l border-gray-200 h-12 mx-4 md:mx-6"></div>
          <div v-if="paquete.precio_paquetes?.[0]?.precio_d" class="text-start flex justify-between items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
              stroke="currentColor" class="size-6 md:size-5">
              <path stroke-linecap="round" stroke-linejoin="round"
                d="M9.568 3H5.25A2.25 2.25 0 0 0 3 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 0 0 5.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 0 0 9.568 3Z" />
