  ...
  <section class=" my-20 container">
    ...
    <carousel :breakpoints="breakpoints" class="-m-3" @update:modelValue="updateProgress">
      ...
      <template #addons="{ currentSlide }">
        <CarouselNavigation>
          <template #prev>
            <div v-if="currentSlide > 0" class="absolute -translate-x-10 p-2 rounded-full border">
          ...
          <template #next>
            <div v-if="progressWidth !== 100" class="absolute translate-x-10  p-2 rounded-full border">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                stroke="currentColor" class="size-6">
